...
describe('Introspecting GraphQL schema documents', () => {
  test(`should generate valid introspection JSON file`, async () => {
    const schemaContents = readFileSync(join(__dirname, '../../test/fixtures/starwars/schema.graphql')).toString();
    const expected = readFileSync(join(__dirname, '../../test/fixtures/starwars/schema.json')).toString();
