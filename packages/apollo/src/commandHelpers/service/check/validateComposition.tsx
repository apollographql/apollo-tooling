...
// Hangups: Do we really need to abstract this `validateComposition` function into it's own file? Does it
// really need to have the mental overhead of using an `updateSteps` function?
//
// Is this better than what we have now with `listr`?

// Why does justin have a hard time understanding listr? The logic for performing work and displaying progress
// is tightly coupled. Going from step to step, you have no idea what's already happened in the past.
...
export async function validateComposition(
  ...
  /**
   * Optional function to update caller with status
   */
  ...
  const validateCompositionSteps: Step[] = [
    fetchPartialSchemaStep,
    composeStep
  ];

  updateSteps(validateCompositionSteps);
