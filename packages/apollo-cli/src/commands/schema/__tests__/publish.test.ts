...
const engineSuccess = (
  { schema, tag } = { schema: localSchema.__schema, tag: "current" }
) => nock => {
  nock
    .matchHeader("x-api-key", ENGINE_API_KEY)
    .post("/", {
      operationName: "UploadSchema",
      variables: { schema, tag: tag || "current" },
...
describe("happy paths", () => {
  test
    ...
    .it("calls engine with a schema from the default remote", ({ stdout }) => {
  ...
  test
    ...
    .it("allows formatting success as json", ({ stdout }) => {
      expect(stdout).toContain('"hash": "12345"');
    });
});
