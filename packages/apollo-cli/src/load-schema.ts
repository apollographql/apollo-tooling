...
export function loadSchemaStep(self: Command, pullFromEngine: boolean, apiKey: string | undefined, engineFlag: string | undefined, notEngineTitle: string, notEngine: (ctx: any) => Promise<void>) {
  return {
    title: pullFromEngine ? "Loading schema from Apollo Engine" : notEngineTitle,
    task: async (ctx: any) => {
      if (pullFromEngine) {
        const variables = {
